...
if [ "$(getprop ro.crypto.state)" == "encrypted" ];then
  # due to mount conflicts which can occur we need to ensure to run
  # ro.twrp.sar is the last property which will be set during TWRP boot up
  #while [ "$(getprop twrp.decrypt.done)" != "true" -a ! -z "$(getprop ro.twrp.sar)" ];do
  while [ "$(getprop twrp.decrypt.done)" != "true" ];do
    F_LOG "device not decrypted, will wait.."
    sleep 1
  done
fi
